...
Encoder.prototype.getEmptyInitSegMessage = function(headerField) {
  ...
  message.writeUIntBE(timecode, 0, 8); 
  message.writeUIntBE(0x07 & padding, 8, 1);
...
Encoder.prototype.getEmptyChunkMessage = function(headerField) {
  ...
  var message = new Buffer(headerSize + payloadSize);
  message.writeUIntBE(timecode, 0, 8); 
  message.writeUIntBE((1 << 6) | (0x07 & padding), 8, 1);
  message.writeUIntBE(chunkIndex, 9, 1);
  message.writeUIntBE(finalIndex, 10, 1);
  message.writeUIntBE(duration, 11, 4);
